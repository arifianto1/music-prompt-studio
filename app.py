...
genre_kombinasi_options = ["Tidak Ada", "Orchestral", "Cyberpunk", "Jazz", "Acoustic", "Metal", "Ambient", "Classical"]
...
produksi_modes = ["Single Track (1 Lagu)", "Kompilasi (Long Duration)"]
...
with col1:
    ...
    genre = st.selectbox("Genre Musik Utama", genres, index=index_or_default(genres, default_genre))
...
# -----------------------------
# LOGIC & STRINGS FROM V2.5.4 & NEW ENGINE
# -----------------------------
genre_rules = {
...
if auto_pilot_90:
    teks_autopilot = "🔥 AUTO PILOT 90+ (OVERRIDE KREATIVITAS): Kamu WAJIB memaksa kualitas aransemen ke level DEWA. Ciptakan transisi tak tertebak, drop ekstrem, dan WAJIB gunakan parameter Hi-Fi tingkat tinggi ('Deep Sub-Bass Resonance', 'Immersive Stereo Panning', 'Sidechained Bassline')."
else:
    teks_autopilot = "Kualitas audio standar produksi profesional."
...
if produksi_mode == "Kompilasi (Long Duration)":
    ...
    2. 1 DESKRIPSI MASTER (dengan template Timestamps: 00:00 Track 1, dst)
    ...
    - LYRICS + STRUCTURE TAGS (in {target_language})
    ...
    4. YOUTUBE SEO TITLES (in {market_language}, Pasar Global Barat)
...
🛡️ ANTI-COPYRIGHT & STRUCTURE RULES (WAJIB PATUHI):
1. Lirik harus 100% ORIGINAL. DILARANG KERAS menyalin lirik, melodi, atau frasa klise lagu komersial. Gunakan metafora teknis (frekuensi, energi, dll).
2. KUNCI STRUKTUR: Pastikan Tag Struktur (seperti [Verse], [Chorus], [Heavy Sub-Bass Drop]) TETAP MENYATU dalam satu blok teks dengan liriknya. JANGAN PERNAH memisahkan tag dari lirik pembentuk suaranya agar render vokal Suno tidak rusak.
